...
public:
	...
	glm::vec3 RayUnProj(float mouseX, float mouseY, float mouseZ, int width, int height) {
		//GLfloat params[4];
		...
		//viewport[2] = params[2];
		//viewport[3] = params[3];
		/*return glm::normalize(glm::unProject(glm::vec3(mouseX, viewport[3] - mouseY, 0.0f), transform.GetModelView(camera), 
		...
		/*return glm::unProject(glm::vec3(mouseX, mouseY, 0.0f), transform.GetModelView(camera),
			camera.GetProjection(), viewport) - glm::unProject(glm::vec3(mouseX, mouseY, 1.0f),
				transform.GetModelView(camera), camera.GetProjection(), viewport);*/
		return glm::unProject(glm::vec3(mouseX, viewport[3]-mouseY, mouseZ), transform.GetModelView(camera),
								camera.GetProjection(), viewport);
	}
